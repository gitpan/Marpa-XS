...
This document is under construction.  As of this release,
...
to allow for the alternative input models.
...
concept.
When input is performed using the recognizer's C<read> method,
the furthest earleme and the current earleme are always the same.
...
In alternative input models,
calling
the recognizer's L<C<earleme_complete>|/"earleme_complete"> method at the
end of input is not enough to ensure that all input has been processed.
The application must also call the
recognizer's L<C<end_input>|/"end_input"> method to ensure
...
input, and is therefore used in alternative input models.
...
B<not> advance the current earleme on each call.
...
The user checking for token
rejection must be careful to test that the return value is undefined,
...
and the current earleme advanced.
...
will use the only token choice that is
