...
value of the node.
A semantic Perl closure is always evaluated in scalar context.
...
Sequence rules nodes are also rule nodes.
...
L<C<default_null_value> named argument|Marpa::XS::Grammar/default_null_value>.
...
If necessary, an application can dedicate a symbol
to service as the left hand side symbol
of the sequence rule -- that is,
make sure that the LHS symbol is not used
for a conflicting purpose elsewhere.
Such a dedicated symbol can then be given
...
a call to the L<Marpa::XS::Recognizer::reset_evaluation
...
When the
L<Marpa::XS::Recognizer::reset_evaluation
method|Marpa::XS::Recognizer/"reset_evaluation"> is called
for a recognizer, it begins a new parse series.
The new parse series continues until
there is another
call to the L<Marpa::XS::Recognizer::reset_evaluation
method|Marpa::XS::Recognizer/"reset_evaluation">,
or until the recognizer is destroyed.
...
the L<Marpa::XS::Recognizer::value
method|Marpa::XS::Recognizer/"value"> of each parse series.
...
Ranking Phases do not occur if the L<ranking method|Marpa::XS::Recognizer/"ranking_method">
is left as the default.
If the ranking method is set to a value other than "C<none>",
a Ranking Phase occurs in the first call
of the L<Marpa::XS::Recognizer::value
method|Marpa::XS::Recognizer/"value"> of each parse series.
...
C<Marpa::XS::Recognizer::value> method.
...
=head2 No Duplication of Parse Results
...
By calling
L<the Marpa::XS::Recognizer::value
method|Marpa::XS::Recognizer/"value">
repeatedly,
...
These duples might, for example,
be implemented as references to arrays.
...
because those rules, if they do not have ranking closures,
...
semantic Perl closures.
The logic that resolves action names to ranking Perl closures is described
L<above|Marpa::XS::Semantics/"RESOLVING ACTION NAMES">.
The ranking Perl closures are called while still in the same
L<Ranking
Phase|Marpa::XS::Semantics/"Ranking Phase">.
...
which return C<undef> will cause all parses to be discarded.

Detecting ambiguous rules is one example
...
argument|Marpa::XS::Grammar/"infinite_action">
...
rewrite can cause a cycle to end before it would end
according to the rules of the original grammar.

Marpa's exact definition of a cycle is experimental
and subject to change.
Applications should avoid
relying on the details
of Marpa's present behavior in determining
...
But the quality of the present implementation
seems to match
or exceed the extent of present interest.
