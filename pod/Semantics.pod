...
semantics are specified as B<action names>.
...
At evaluation time,
action names
are resolved to actions,
which are Perl closures.
Action names and actions are used for producing
node values, for ranking nodes,
and in creating the per-parse variable.
...
The Perl closure that is the value action
of a sequence rule node is written in
the same way
...
They do not appear in its C<@_> array.
...
=head3 The Default Null Semantics
...
=head3 More Complex Null Semantics

This document contains all that most
users will need to know about implementing their
null semantics.
Users who need to implement a very complex null semantics,
or who simply would like
to read a more detailed account of how Marpa's
null semantics works,
can turn to L<the document on
null semantics|Marpa::XS::Semantics::Null>.

An application can assign null values to symbols.
If the null node is for a symbol, its null value
is the null value of its symbol.
A null node can be for a rule.
In that case, its null value is the null value of
the rule's LHS.

An application can also change a grammar's
default null value,
which is C<undef>, to a constant,
using the grammar's
L<C<default_null_value>|Marpa::XS::Grammar/default_null_value>
named argument.

Specifying null semantics on a per-symbol basis is a
natural way to do it, and the result is essentially what
the user would expect.
Several special issues should be watched for.

=head3 Nulled Subtrees

Basing the null value on the node's symbol implies
that, in a null subtree, the semantics of the topmost null
node will be the only thing that matters.
In computing its null value, a node never takes into
account the values of its child nodes.

=head3 Nullable Rules

Nullable rules derive their values from two different
sources, depending on whether they are nulled or not.
If a nullable rule node B<is not> nulled, its value comes from the
semantics of its rule.
If a nullable rule node B<is> nulled, its values is the null value
of the rule's LHS symbol.

It is up to the application to make sure that, for nullable rules,
the rule semantics and the null value of the LHS symbol
"make sense" when used together in an application.
This can always be done, if necessary by making sure the LHS
of the nullable rule is a dedicated symbol -- one that
serves that purpose and that purpose only.
A LHS symbol can be regarded as dedicated if it
is not a terminal and is not
the LHS of any other rule.

=head3 Nullable Sequence Rules

The remarks just made about nullable rules apply to nullable sequence
rules.
A sequence rule is nullable if
the minimum number of items in it is zero.
A nullable sequence which is nulled has the null value of its LHS symbol.
A nullable sequence which is not nulled has the semantics of its rule.

=head1 AN OVERVIEW OF MARPA'S SEMANTICS

The null semantics uses constants.
The rest of Marpa's semantics is dynamic.
Marpa's dynamic semantics is implemented using action names,
which it later resolves to Perl closures.
To implement its dynamic semantics,
Marpa does three things:
...
=item * Call the Perl closure to produce a value.
...
until the Tree Traversal Phase.
...
recognizer's L<ranking_method|Marpa::XS::Recognizer/"ranking_method">
...
If the the action object constructor name is
a hash key in the
...
Marpa allows grammar with infinite loops.
These are universally considered useless in practical applications,
and Marpa supports all the semantics for these that should be necessary
and more.
Marpa allows infinite loops because doing so allows it to accurately
...
Where the break occurs varies among Marpa implementations.
...
For those curious,
further details can be found in a L<separate
