...
Marpa::XS::Debug - Marpa Grammar Debugging
...
This is called the "current earleme".
In our example the current earleme is earleme 3.
You can find out the current earleme using the
L<C<current_earleme> method|Marpa::XS::Recognizer/"current_earleme">.
But you can also simply call
L<C<show_progress>|Marpa::XS::Recognizer/"show_progress">
without arguments.
By default,
L<C<show_progress>|Marpa::XS::Recognizer/"show_progress">
...
earleme numbers, separated by a hyphen.
The first earleme number is the origin, the place where Marpa
first started recognizing the rule.
The last earleme number is the dot earleme, the earleme location
of the dot in a dotted rule.
"C<@0-3>" say that this rule began at earleme 0, and that the
dot is at location 3.

Earleme 3 is the current earleme and this is no coincidence.
Whenever we are displaying the progress report for an earleme,
...
progress report indicates that if our input ended at earleme 3, it would be a valid sentence
...
its origin at earleme 0, and the other has its origin
at earleme 2.
...
When there are many
instances of a dotted rule at a single earleme location,
it may be inconvenient to show all the origins in a comma-separated
list.
In that case the origins in the location field are shown as a
range, with the earliest separated from the most recent
by a "C<...>".
The example is this document contains no lines with
a large number of
...
at the earleme where things went wrong:
...
The answer is that 
there should be a prediction of "C<Term -E<gt> Term Add Term>" at earleme 0.
...
To be precise, Marpa allows exactly one nulling rule,
but this is a rule that it creates internally,
