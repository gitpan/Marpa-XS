...
document|Marpa::XS::Semantics/"Sequence Rule Nodes">.
...
Details are in
L<the document on semantics|Marpa::XS::Semantics>.
...
For details
see
L<the document on Marpa's semantics|Marpa::XS::Semantics>
...
Note that
a sequence rule and a non-sequence rule are considered
duplicates if both have
identical left and right hand sides.
Similarly, two sequence rules
can be duplicates even if they have
have different separators and/or different counts.
Sequence separators and counts
play no role
in determining if the rule is a duplicate.
