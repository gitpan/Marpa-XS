...
In XS mode, this call is a no-op.
In Pure Perl mode, when called after
input is finished,
this call cleans up internal data
not needed
for the evaluation phase.

In Pure Perl mode,
A considerable amount of data is only used while building
the recognizer's tables, and is not needed once they are complete.
