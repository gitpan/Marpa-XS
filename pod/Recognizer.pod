...
C<read> returns a Perl C<undef>.

The C<read> method implements Marpa's default input model,
which is all many users will be interested in.
Marpa allows other, non-traditional models of the input.
...
By default, when a token is rejected,
the
L<C<Marpa::XS::Recognizer::read>|/read> call returns a
Perl C<undef>.
At this point, an application can always retry input,
using a different token.
The L<C<Marpa::XS::Recognizer::expected_terminals>|/expected_terminals> call
returns the list of tokens that <b>will</b> be accepted at any point.
