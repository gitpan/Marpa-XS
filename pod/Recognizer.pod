...
In Marpa, locations in the input stream are also called B<earlemes>.
The B<current earleme> means exactly the same thing as
...
Any application that restricts itself to reading input
using the methods
described in this document will be using the default, token-stream,
model of input.
...
acceptable at the current earleme.
...
(or current earleme).
It then advances the current earleme by 1.
...
the number of tokens which are acceptable at the new current earleme.
...
and is always the number of the earleme which begins the range.
If both arguments are non-negative integers,
the range is from earleme I<N> to earleme I<M>.
If the second argument is a negative integer,
I<-M>,
the end of the range is the 
I<M>th location from the furthest earleme.
For example, if 42 was the furthest earleme,
-1 would be earleme 42 and -2 would be earleme 41.
...
to
specify ranking actions which assign values to rules and tokens.
These values will
control the order in which parse results are returned by
the C<value> method.
...
An application which is doubtful about the acceptability
of a token does not have to check
the output of C<terminals_expected>.
It is a recoverable error
if a token is rejected because it is not acceptable.
If an application is not sure whether a token is
acceptable or not, the application can simply
attempt to input the dubious token using
