...
sub token_not_accepted {
    ...
    say STDERR $Marpa::XS::Perl::RECOGNIZER->show_progress();
    my $perl_token_desc;
...
sub unknown_ppi_token {
	...
	'Marpa::XS::Perl did not know how to process token',
	Marpa::XS::Perl::default_show_location($ppi_token), "\n"
}

sub Marpa::XS::Perl::parse {
    ...
    # error messages
    local $Marpa::XS::Perl::RECOGNIZER = $recce;
    ...
    local $Marpa::XS::Perl::Internal::CONTEXT =
            ...
            for my $cast ( split //xms, $content ) {
                ...
                if ( not defined $perl_type ) {
                    die qq{Unknown $PPI_type: "$content":},
                        Marpa::XS::Perl::default_show_location($token),
                        "\n";
                }
                $token_found = 1;
...
} ## end sub Marpa::XS::Perl::parse

# Context-sensitive callback for
# application-provided closures to use.
sub Marpa::XS::Perl::token {
    Marpa::XS::exception('No Perl context for token callback')
        if not my $context = $Marpa::XS::Perl::Internal::CONTEXT;
    my ( $PPI_tokens, $earleme_to_token ) = @{$context};
    my $earleme = Marpa::XS::location();
    return $PPI_tokens->[ $earleme_to_token->[$earleme] ];
} ## end sub Marpa::XS::Perl::token

sub Marpa::XS::Perl::default_show_location {
    my ($token) = @_;
    ...
    return
        ...
        . q{, column }
        . $token->column_number();
} ## end sub Marpa::XS::Perl::default_show_location
